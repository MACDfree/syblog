...
func (r *FormatRenderer) renderImage(node *ast.Node, entering bool) ast.WalkStatus {
	if entering && r.DisableTags == 0 {
		destTokens := node.ChildByType(ast.NodeLinkDest).Tokens
		destTokens = r.LinkPath(destTokens)
		imagePath := string(html.EscapeHTML(destTokens))
		if strings.HasPrefix(imagePath, "assets/") {
			r.article.Asserts = append(r.article.Asserts, strings.TrimPrefix(imagePath, "assets/"))
		}
	}
	return ast.WalkContinue
}
...
func (r *FormatRenderer) renderLink(node *ast.Node, entering bool) ast.WalkStatus {
	if entering {
		...
		if node.LinkType == 0 {
			textNode := node.ChildByType(ast.NodeLinkText)
			if textNode != nil {
				text := util.BytesToStr(textNode.Tokens)
				text = strings.Trim(text, "\"")
				r.WriteString("[" + text + "]")
			}
			urlNode := node.ChildByType(ast.NodeLinkDest)
			if urlNode != nil {
				link := util.BytesToStr(urlNode.Tokens)
				if strings.HasPrefix(link, "siyuan://blocks/") {
					// 需要根据块ID找到对应的文章，替换为对应文章的路径，并且需要导出此文章
				...
				r.WriteString("(" + link)
				titleNode := node.ChildByType(ast.NodeLinkTitle)
				if titleNode != nil {
					title := util.BytesToStr(textNode.Tokens)
					r.WriteString(" \"" + title + "\"")
				}
				r.WriteString(")")
			}
...
func (r *FormatRenderer) renderText(node *ast.Node, entering bool) ast.WalkStatus {
	if entering {
		...
		if (nil == node.Previous || ast.NodeTaskListItemMarker == node.Previous.Type) &&
			nil != node.Parent.Parent && nil != node.Parent.Parent.ListData && node.Parent.Parent.ListData.Typ == 3 {
			if r.LastOut == ' ' {
...
func (r *FormatRenderer) renderBlockquote(node *ast.Node, entering bool) ast.WalkStatus {
	if entering {
		...
		if len(r.NodeWriterStack) == 1 { // 已经是根这一层
			length = len(lines)
...
func (r *FormatRenderer) renderListItem(node *ast.Node, entering bool) ast.WalkStatus {
	if entering {
		...
		if node.ListData.Typ == 1 || (node.ListData.Typ == 3 && node.ListData.BulletChar == 0) {
			indent++
		}
...
func (r *FormatRenderer) withoutKramdownBlockIAL(node *ast.Node) bool {
	return !r.Options.KramdownBlockIAL || len(node.KramdownIAL) == 0 || nil == node.Next || ast.NodeKramdownBlockIAL != node.Next.Type
}

func (r *FormatRenderer) tagSrcPath(tokens []byte) []byte {
...
func (r *FormatRenderer) setextHeadingLen(node *ast.Node) (ret int) {
	buf := &bytes.Buffer{}
